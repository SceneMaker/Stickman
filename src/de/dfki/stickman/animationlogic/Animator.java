...
public class Animator {
	...
	private void renderEventAnimation() {
		for (ArrayList<Entry> cluster : mWTS.getClusters()) {
			// mStickman.mLogger.info("Cluster is a " +
			// WordTimeMarkSequence.getClusterType(cluster).name());
			if (WordTimeMarkSequence.getClusterType(cluster) == Entry.TYPE.WORD) {
				...
				// mStickman.mLogger.info("Animator - animation " + mAnimation +
				// " render pause " + mRenderPauseDuration + " duration " +
				// duration);
				render();
			}
	...
	private void render() {
		...
		while (mCurrentStep > 0) {
			// for (mCurrentStep = sMAX_ANIM_STEPS; mCurrentStep > 0;
			// mCurrentStep--) {
			// DEBUG mStickman.mLogger.info("currentstep " + mCurrentStep + "
			// max steps " + sMAX_ANIM_STEPS);
