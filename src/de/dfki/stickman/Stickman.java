...
public class Stickman extends JComponent {
	...
	public void playAnimation(Animation a) {
		try {
			// mLogger.info("Waiting for allowance to play animation " +
			// a.toString());
			mAnimationLaunchControl.acquire();
	...
	protected void paintComponent(Graphics g) {
		...
		at.rotate(Math.toRadians(mWobble), (mBody.getRightLegStartPostion().x + mBody.getLeftLegStartPostion().x) / 2,
				mBody.getRightLegStartPostion().y + mLeftLeg.mLength);
		...
		if (starShowControler == true)
			...
			if (starShowC == true)
				mStars.update(g); // Added by Robbie, to show stars or words
									// here.
		}
