...
public class StickmanStage extends JFrame implements MouseListener {
	...
	private StickmanStage() {
		...
		setDefaultCloseOperation(WindowConstants.HIDE_ON_CLOSE); // This one is
																	// needed by
																	// SceneMaker
		...
		if (!sStickmansOnStage.containsKey(name.toLowerCase())) {
			if (sFullScreen) {
				sStickmansOnStage.put(name.toLowerCase(),
						new Stickman(name, gender, mHeight / (float) Stickman.mDefaultSize.height * sScale,
								new Dimension(new Float(mHeight * 2 / 3 * sScale).intValue(),
	...
	public static void parseStickmanMLCmd(String cmd) {
		...
		if (stickmanname != null) {
			a = (a instanceof EventAnimation)
					? AnimationLoader.getInstance().loadEventAnimation(getStickman(stickmanname), animationname,
							duration, blocking)
					: AnimationLoader.getInstance().loadAnimation(getStickman(stickmanname), animationname, duration,
							blocking);
	...
	public static void main(String[] args) {
		// ExtensionsFromJar e = new
		// ExtensionsFromJar("de.dfki.stickman.animation.face",false);
		// e.loadClass();
	...
	public void mouseClicked(MouseEvent e) {
		...
		// getStickman("Anna").doAnimation("Speaking", 3000, "Stell Dir vor, Du
		// kommst nach Hause, und ein Pferd steht in der Küche.", false);
		// getStickman("Anna").doAnimation("Speaking", 3000, "Stell Dir vor, Du
		// kommst nach Hause, und ein Pferd steht in der Küche.", false);
		...
		getStickman("Anna").doAnimation("DisappearToSmall", 300, true);
		...
		// getStickman("Anna").doAnimation("head", "Blink", true);
		//// //smM.doAnimation("environment", "speak", "Stell Dir vor, Du kommst
		// nach Hause, und ein Pferd steht in der Küche.", false);
		// getStickman("Anna").doAnimation("environment", "speak", 300, "Stell
		// Dir vor, Du kommst nach Hause, und ein Pferd steht in der Küche.",
		// false);
