package de.dfki.stickman.environment;

import de.dfki.stickman.animationlogic.Animator;
import de.dfki.stickman.body.BodyPart;
...
public class SpeechBubble extends BodyPart {
	...
	public void createShape() {
		...
		case SPEAK:
			Point upperBubbleSpikeStart = new Point(mStart.x + mHead.mSize.width / 3,
					mStart.y - mHead.mSize.height / 3 - 10);
			...
			mBubble.quadTo((mStart.x + upperBubbleSpikeStart.x) / 2 + 10, (mStart.y + upperBubbleSpikeStart.y) / 2 + 10,
					upperBubbleSpikeStart.x, upperBubbleSpikeStart.y);
			mBubble.curveTo(upperBubbleSpikeStart.x - mLength / 3, upperBubbleSpikeStart.y - 10, topSeam.x - mLength,
					topSeam.y, topSeam.x, topSeam.y);
			mBubble.curveTo(topSeam.x + mLength, topSeam.y, bottomSeam.x + mLength, bottomSeam.y, bottomSeam.x,
					bottomSeam.y);
			mBubble.quadTo((lowerBubbleSpikeStart.x + bottomSeam.x) / 2,
					(lowerBubbleSpikeStart.y + bottomSeam.y) / 2 + 3, lowerBubbleSpikeStart.x, lowerBubbleSpikeStart.y);
			mBubble.quadTo((mStart.x + lowerBubbleSpikeStart.x) / 2 + 10, (mStart.y + lowerBubbleSpikeStart.y) / 2 + 10,
					mStart.x, mStart.y);
			break;
		}
	}
	...
	protected void paintComponent(Graphics g) {
		...
		if (mShape != SHAPE.DEFAULT) {
			...
			if (endB > startB) {
				// mHead.mStickman.mLogger.info("total >" + mText + "< current
				// >" + mCurrentlySpokenText + "< start " + startB + " end " +
				// endB);
				// attributedString.addAttribute(TextAttribute.UNDERLINE,
				// TextAttribute.UNDERLINE_ON, startB, endB);
				// attributedString.addAttribute(TextAttribute.INPUT_METHOD_UNDERLINE,
				// TextAttribute.UNDERLINE_LOW_TWO_PIXEL, startB, endB);
		...
		if (mCurrentlySpokenText.isEmpty()) {
			g2.setColor(new Color(0, 0, 0,
					64 - (Animator.sMAX_ANIM_STEPS - (new Double(mShapeAnimationStep)).intValue()) * 3));
		} else {
