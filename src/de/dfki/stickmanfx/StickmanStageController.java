...
//	handleStickman();
	...
	ExitButton.setOnAction(new EventHandler<ActionEvent>() {
		...
		((StickmanStageFX) mStickmanOnstage.getStageStickman()).clearStage(
			((StageStickmanControllerFX) mStickmanOnstage.getStageStickmanController()).CONFIG_STAGE);
	    }
	});
...
//	fillComboForStickman();
	...
	if (mStickmanOnstage.getmFilePath() != null)
	    filexml = new File(
		    mStickmanOnstage.getmFilePath() + File.separator + "stickman" + File.separator + "stickman.xml");
	else {
	    try {
		filexml = new File(new File(".").getCanonicalPath() + File.separator + "stickman" + File.separator
			+ "stickman.xml");
	    } catch (IOException e) {
		e.printStackTrace();
	    }
