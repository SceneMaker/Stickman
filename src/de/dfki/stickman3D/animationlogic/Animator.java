...
import de.dfki.stickman3D.util.TimingInfo;
...
public class Animator {
	...
	public Animator(Stickman3D sm, Animation a, ArrayList<AnimationContent> animComps) {
		mStickmanFX = sm;
		mAnimationFX = a;
		mAnimationComponents = animComps;
		mDescription = mAnimationFX.getClass().getSimpleName() + " (" + mAnimationFX.mID + "), "
				+ mAnimationFX.toString();
		mRenderPauseDuration = 40; // 40 milliseconds equals 25fps - resulting
									// that by default an animation takes 500ms

		render();
	}

	public Animator(Stickman3D sm, Animation a, ArrayList<AnimationContent> animComps, int duration) {
		mStickmanFX = sm;
		mAnimationFX = a;
		mAnimationComponents = animComps;
		mDescription = mAnimationFX.getClass().getSimpleName() + " (" + mAnimationFX.mID + "), "
		...
		mRenderPauseDuration = (mRenderPauseDuration < 1) ? 1 : mRenderPauseDuration; // minimum
																						// delay
																						// is
																						// 1
																						// millisecond
	...
	public Animator(Stickman3D sm, Animation a, ArrayList<AnimationContent> animComps, int duration, int step) {
		mStickmanFX = sm;
		mAnimationFX = a;
		mAnimationComponents = animComps;
		mDescription = mAnimationFX.getClass().getSimpleName() + " (" + mAnimationFX.mID + "), "
	...
	public Animator(Stickman3D sm, Animation a, ArrayList<AnimationContent> animComps, WordTimeMarkSequence wts) {
		mStickmanFX = sm;
		mAnimationFX = a;
		mAnimationComponents = animComps;
		mWTS = wts;
	...
	private void renderEventAnimation() {
		for (ArrayList<Entry> cluster : mWTS.getClusters()) {
			if (WordTimeMarkSequence.getClusterType(cluster) == Entry.TYPE.WORD) {
				...
				currentlySpokenText = text.replace("oe", "ö").replace("ae", "ä").replace("ue", "ü")
						.replace("Oe", "Ö").replace("Ae", "Ä").replace("Ue", "Ü").replace("ss", "ß")
						.replace("\n", " ").replace("   ", " ").replace("  ", " ");

				String allText = "";
				allText = mWTS.getText().replace("oe", "ö").replace("ae", "ä").replace("ue", "ü").replace("Oe", "Ö")
						.replace("Ae", "Ä").replace("Ue", "Ü").replace("ss", "ß").replace("\n", " ")
						.replace("   ", " ").replace("  ", " ");

				// mStickmanFX.mSpeechBubble.mText = allText;
				// mStickmanFX.mSpeechBubble.mCurrentlySpokenText =
				// currentlySpokenText;
			...
			if (WordTimeMarkSequence.getClusterType(cluster) == Entry.TYPE.TIMEMARK) {
				// here we have to spread the word that a specific timemark has
				// been reached
				// the interface is the runActionAtTimemark method in the
				// EventActionPlayer
				for (Entry e : cluster) {
					// we have 2 options!
					// 1) API Call
		...
		while (mCurrentStep > 0) {
			if (mCurrentStep == sMAX_ANIM_STEPS) {
				...
				mAnimationComponents.stream().forEach((comp) -> {
					BodyPart bodypartFX = comp.mBodyPartFX;
			...
			if (mCurrentStep > 1) {
				for (AnimationContent ba : mAnimationComponents) {
					BodyPart bodypartFX = ba.mBodyPartFX;
